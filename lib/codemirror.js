...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function onCopy(e) {
      var txt = input.value = getSelection();
      leaveInputAlone = true;
      setSelRange(input, 0, txt.length);
    ...
    function readInput() {
      ...
      var text = input.value, range = selRange(input);
      if (!text) return false;
      shiftSelecting = null;
      input.value = "";
      replaceSelection(text, !range.end ? "start" : range.start ? "end" : false);
      return true;
    }
