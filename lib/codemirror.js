...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    connect(input, "cut", onCutCopy);
    connect(input, "copy", onCutCopy);
    ...
    function onCutCopy(e) {
      input.value = getSelection();
      leaveInputAlone = true;
      input.select();
      if (e.type == "cut") operation(replaceSelection)("");
      setTimeout(function() {resetInput(); leaveInputAlone = false;}, 20);
    }
    ...
    function onKeyDown(e) {
      ...
      lastStoppedKey = handleKeyBinding(e) ? e.keyCode : null;
    }
    function onKeyPress(e) {
      if (e.keyCode == lastStoppedKey) {lastStoppedKey = null; e_preventDefault(e); return;}
      if (options.onKeyEvent && options.onKeyEvent(instance, addStop(e))) return;
      if (handleKeyBinding(e)) return;
    ...
    var prevInput = "";
    function readInput() {
      if (leaveInputAlone || !focused) return false;
    ...
    function setSelection(from, to, oldFrom, oldTo) {
      ...
      if (prevInput && from.line != oldFrom) resetInput();
  ...
  // TODO insert/overwrite, search/searchnext/replace, fold/unfold
  var commands = CodeMirror.commands = {
    ...
    goLineStart: function(cm) {
      ...
      if (cm.getOption("smartHome")) {
        var text = cm.getLine(cur.line), firstNonWS = Math.max(0, text.search(/\S/));
        cm.setCursor(cur.line, cur.ch <= firstNonWS && cur.ch ? 0 : firstNonWS, true);
      } else {
        cm.setCursor(cur.line, 0, true);
      }
    },
    ...
    indentAuto: function(cm) {cm.indentSelection("auto");},
  ...
  var keyMaps = CodeMirror.maps = {};
  keyMaps.basic = {
    "Left": "goCharLeft", "Right": "goCharRight", "Up": "goLineUp", "Down": "goLineDown",
    "End": "goLineEnd", "Home": "goLineStart", "PageUp": "goPageUp", "PageDown": "goPageDown",
  ...
  keyMaps.pcDefault = {
    ...
    "Ctrl-Backspace": "delWordLeft", "Ctrl-Delete": "delWordRight", fallthrough: "basic"
  };
  keyMaps.macDefault = {
    ...
    "Ctrl-Alt-Backspace": "delWordRight", "Alt-Delete": "delWordRight", fallthrough: ["basic", "emacsy"]
  };
  keyMaps["default"] = mac ? keyMaps.macDefault : keyMaps.pcDefault;
  keyMaps.emacsy = {
    "Ctrl-F": "goCharRight", "Ctrl-B": "goCharLeft", "Ctrl-P": "goLineUp", "Ctrl-N": "goLineDown",
    "Alt-F": "goWordLeft", "Alt-B": "goWordRight", "Ctrl-A": "goLineStart", "Ctrl-E": "goLineEnd",
  ...
  function lookupKey(event, extraMap, map) {
    ...
    function lookup(name, map, ft) {
      ...
      if (typeof ft == "string") return lookup(name, keyMaps[ft]);
      for (var i = 0, e = ft.length; i < e; ++i) {
        found = lookup(name, keyMaps[ft[i]]);
        if (found != null) return found;
      }
      return null;
    }
    function lookup0(name) {
      return extraMap ? lookup(name, extraMap, map) : lookup(name, keyMaps[map]);
    }
    if (event.shiftKey) {
