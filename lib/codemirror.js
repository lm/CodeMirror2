...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function onCopy(e) {
      ...
      input.select();
      setTimeout(function() {input.value = ""; leaveInputAlone = false;}, 20);
    }
    function onKeyDown(e) {
      ...
      var bound = lookupKey(e, options.keyMap);
      if (bound != null) {
        e_preventDefault(e);
        commands[bound](instance);
        return;
      }
    }
    function onKeyUp(e) {
    ...
    function onKeyPress(e) {
      ...
      // Re-stop enter. Necessary on some browsers.
      if (e.keyCode == 13) {if (!options.readOnly) handleEnter(); e_preventDefault(e);}
      else {
    ...
    function onFocus() {
      ...
      if (!focused) {
        ...
        if (!leaveInputAlone) input.value = "";
      }
      slowPoll();
      restartBlink();
    }
    ...
    function readInput() {
      if (leaveInputAlone || !focused || options.readOnly) return false;
      var text = input.value;
      if (!text) return false;
      shiftSelecting = null;
      replaceSelection(text, hasASelection(input) ? false : "end");
      input.value = "";
      return true;
    }
  ...
  // TODO catch mutating commands when readOnly is true
  ...
  function lookupKey(event, map) {
    ...
    function lookup(name, map) {
      var m = keyMaps[map], found = m[name];
      if (found != null) return found;
      if (m.fallthrough)
        for (var i = 0; i < m.fallthrough.length; ++i) {
          found = lookup(name, m.fallthrough[i]);
          if (found != null) return found;
        }
      return null;
    }
    if (event.shiftKey) {
      var found = lookup("Shift-" + name, map);
      if (found != null) return found;
    }
  ...
  // See whether a textarea has selected text
  var hasASelection = window.getSelection ? function(te) {
    try {return te.selectionEnd != te.selectionStart;}
    catch(e) {}
  } : function(te) {
